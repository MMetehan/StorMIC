...
> **You need to run your own signaling server.** See [`server/README.md`](../server/README.md) for setup instructions.
...
StorMIC requires a signaling server to work. The server is a tiny stateless WebSocket relay — no accounts, no storage, no message history. See **[`server/README.md`](../server/README.md)** for:
...
> **Kendi sinyal sunucunu çalıştırman gerekir.** Kurulum talimatları için [`server/README.md`](../server/README.md) dosyasına bak.
...
StorMIC'in çalışması için bir sinyal sunucusu gereklidir. Sunucu; küçük, durumsuz bir WebSocket aktarıcısıdır — hesap yok, depolama yok, mesaj geçmişi yok. Aşağıdakiler için **[`server/README.md`](../server/README.md)** dosyasına bak:
